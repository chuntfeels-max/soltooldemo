...
import { isValidSolanaAddress, formatAddress } from './utils/validation';
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-blue-600/30">
      ...
      <main className="max-w-7xl mx-auto w-full p-6 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-9 space-y-8">
          <form onSubmit={handleSearch} className="relative group">
            ...
            <input type="text" className="w-full bg-slate-900/40 border border-white/5 rounded-2xl py-4 pl-12 pr-4 text-sm font-mono focus:border-blue-500/30 outline-none transition-all shadow-xl" placeholder={t.search.placeholder} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            <button type="submit" className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-blue-500 hover:text-white uppercase transition-colors">Enter</button>
          </form>

          {/* 资产 HUD */}
          <div className="bg-gradient-to-br from-blue-900/20 to-transparent border border-white/5 rounded-[2rem] p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 shadow-2xl relative overflow-hidden">
             <div className="absolute top-0 right-0 p-8 text-blue-500/5"><Wallet className="w-32 h-32" /></div>
             <div className="space-y-4 relative z-10 w-full md:w-2/3">
                <div className="flex items-center gap-3">
                  <span className="px-2 py-0.5 bg-blue-600/10 border border-blue-500/20 rounded text-[8px] font-black text-blue-400 uppercase tracking-widest">Active Intelligence</span>
                  {/* 数据源指示灯 */}
                  <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded border text-[8px] font-black uppercase tracking-tighter ${isDataMock ? 'bg-orange-500/10 text-orange-500 border-orange-500/20' : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'}`}>
                    <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${isDataMock ? 'bg-orange-500' : 'bg-emerald-500'}`} />
                    {isDataMock ? 'Demo Mode (Mock)' : 'Live On-Chain'}
                  </div>
                  <button onClick={() => {navigator.clipboard.writeText(activeAddress); setCopySuccess(true); setTimeout(()=>setCopySuccess(false), 2000)}} className="text-slate-600 hover:text-white transition-colors">
                    {copySuccess ? <CheckCircle2 className="w-4 h-4 text-emerald-500" /> : <Copy className="w-4 h-4" />}
                  </button>
                </div>
                <h2 className="text-xl md:text-2xl font-black font-mono tracking-tighter text-white break-all leading-tight">
                  {activeAddress}
                </h2>
             </div>
             <div className="flex gap-12 items-center relative z-10 shrink-0">
                <div className="text-right">
                  <div className="flex items-center gap-3 justify-end mb-1">
                    <p className="text-[10px] text-slate-500 uppercase font-black">Portfolio Value</p>
                    <button
                      onClick={handleRefresh}
                      disabled={isLoading}
                      className="p-1.5 rounded-lg bg-slate-800/50 hover:bg-slate-700/50 text-slate-400 hover:text-blue-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                      title={lang === 'zh' ? '刷新数据（强制重新获取）' : 'Refresh data (force reload)'}
                    >
      ...
      {showBotModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
          <div className="absolute inset-0 bg-slate-950/90 backdrop-blur-md" onClick={() => setShowBotModal(false)} />
          <div className="relative bg-slate-900 border border-white/10 w-full max-w-sm rounded-[2.5rem] p-10 shadow-3xl text-center space-y-8 animate-in zoom-in duration-200">
            <div className="w-20 h-20 bg-blue-600/20 text-blue-500 rounded-3xl flex items-center justify-center mx-auto ring-1 ring-blue-500/20"><Send className="w-10 h-10" /></div>
            <div className="space-y-2">
              <h2 className="text-2xl font-black text-white tracking-tight italic uppercase">{t.modal.title}</h2>
              <p className="text-slate-500 text-xs leading-relaxed font-medium">{t.modal.desc}</p>
            </div>
            <button onClick={() => setShowBotModal(false)} className="w-full py-4 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-2xl transition-all shadow-xl shadow-blue-600/30 text-[10px] uppercase tracking-[0.2em]">{t.modal.button}</button>
          </div>
        </div>
      )}
